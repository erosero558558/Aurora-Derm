...
function hasOverlap(leftFiles = [], rightFiles = []) {
    ...
    const right = new Set((rightFiles || []).map(normalizePath).filter(Boolean));
...
function inferCodexLane(task) {
    ...
    if (isCritical || inferred.hasCrossDomainFiles || inferred.lane === 'backend_ops') {
        return {
...
function migrateBoard(board) {
    ...
    for (const task of board.tasks || []) {
        ...
        const hasBlockingActiveOverlap = (board.tasks || []).some((otherTask) => {
            if (otherTask === task) return false;
            if (!ACTIVE_STATUSES.has(String(otherTask.status || '').trim().toLowerCase())) {
                return false;
            }
            if (
                String(otherTask.codex_instance || '').trim().toLowerCase() !==
                String(nextTask.codex_instance || '').trim().toLowerCase()
            ) {
                return false;
            }
...
function main(argv = process.argv.slice(2)) {
    ...
    const currentRevision = Number.parseInt(String(board?.policy?.revision || '0'), 10);
    ...
    const report = {
        ...
        next_revision:
            options.apply && changes.length > 0 ? currentRevision + 1 : currentRevision,
