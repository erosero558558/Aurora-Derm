...
function validateTurneroClinicProfile(profile, options = {}) {
    ...
    if (normalized.release.admin_mode_default !== 'basic') {
        errors.push(
            `${sourceLabel}: release.admin_mode_default debe quedar en basic para el piloto web`
        );
    }
    if (normalized.release.native_apps_blocking) {
        errors.push(
...
async function verifyRemoteTurneroClinicProfile(baseUrl, options = {}) {
    ...
    if (turneroPilot) {
        ...
        if (turneroPilot.nativeAppsBlocking !== false) {
            errors.push(
                'checks.turneroPilot.nativeAppsBlocking debe ser false.'
            );
        }
