...
function normalizeTurneroClinicProfile(rawProfile) {
    ...
    return {
        ...
        branding: {
            name: normalizeString(branding.name, 'Aurora Derm'),
            short_name: normalizeString(
                branding.short_name,
                normalizeString(branding.name, 'Aurora Derm')
            ),
        ...
        release: {
            mode: normalizeString(release.mode, 'suite_v2'),
            admin_mode_default:
...
function validateTurneroClinicProfile(profile, options = {}) {
    ...
    const releaseMode = normalized.release.mode;
    const supportedReleaseMode =
        releaseMode === 'web_pilot' || releaseMode === 'suite_v2';
    ...
    if (normalized.release.admin_mode_default !== 'basic') {
        errors.push(
            `${sourceLabel}: release.admin_mode_default debe quedar en basic para el turnero`
        );
    }
    if (!supportedReleaseMode) {
        errors.push(
            `${sourceLabel}: release.mode invalido (${normalized.release.mode})`
        );
    }
    if (
        normalized.release.mode === 'web_pilot' &&
        normalized.release.native_apps_blocking
    ) {
        errors.push(
            `${sourceLabel}: release.native_apps_blocking debe quedar en false para el piloto web`
        );
    }
    if (
        normalized.release.mode === 'suite_v2' &&
        !normalized.release.native_apps_blocking
    ) {
        errors.push(
            `${sourceLabel}: release.native_apps_blocking debe quedar en true para suite_v2`
        );
    }
...
function resolveDiagnosticsAccess(options = {}) {
    const token = normalizeString(
        options.diagnosticsToken ||
            process.env.PIELARMONIA_DIAGNOSTICS_ACCESS_TOKEN ||
            process.env.PIELARMONIA_CRON_SECRET
    );
    if (!token) {
        return {
            enabled: false,
    ...
    const headerName = normalizeString(
        options.diagnosticsHeader ||
            process.env.PIELARMONIA_DIAGNOSTICS_ACCESS_TOKEN_HEADER,
        'Authorization'
    );
    const prefix = normalizeString(
        options.diagnosticsPrefix ||
            process.env.PIELARMONIA_DIAGNOSTICS_ACCESS_TOKEN_PREFIX,
        'Bearer'
    );
...
async function verifyRemoteTurneroClinicProfile(baseUrl, options = {}) {
    ...
    const localFingerprint = localProfile
        ...
        : '';
    const diagnosticsAccess = resolveDiagnosticsAccess(options);
    const remoteUrl = new URL(
        `/api.php?resource=${diagnosticsAccess.resource}`,
        normalizedBaseUrl
    ).toString();
    ...
    try {
        response = await requestJson(remoteUrl, {
            ...
            headers: diagnosticsAccess.headers,
        });
    } catch (error) {
        return {
            ...
            publicSync: null,
            turneroPilot: null,
            diagnosticsAuthorized: diagnosticsAccess.enabled,
            remoteResource: diagnosticsAccess.resource,
        };
    }
    ...
    const healthLooksOk =
        Boolean(payload) && payload.ok === true && payload.status === 'ok';
    const publicHealthRedacted =
        !diagnosticsAccess.enabled &&
        healthLooksOk &&
        turneroPilot === null &&
        publicSync === null;
    ...
    if (!turneroPilot && !publicHealthRedacted) {
    ...
    if (turneroPilot) {
        ...
        const expectedReleaseMode = String(
            localProfile?.release?.mode || ''
        ).trim();
        const expectedNativeAppsBlocking =
            localProfile?.release?.native_apps_blocking === true;
        if (
            expectedReleaseMode &&
            String(turneroPilot.releaseMode || '').trim() !==
                expectedReleaseMode
        ) {
            errors.push(
                `checks.turneroPilot.releaseMode debe ser ${expectedReleaseMode}.`
            );
        }
        ...
        if (
            typeof localProfile?.release?.native_apps_blocking === 'boolean' &&
            turneroPilot.nativeAppsBlocking !== expectedNativeAppsBlocking
        ) {
            errors.push(
                `checks.turneroPilot.nativeAppsBlocking debe ser ${expectedNativeAppsBlocking ? 'true' : 'false'}.`
            );
        }
    ...
    if (publicHealthRedacted) {
        warnings.push(
            'Health remoto redactado sin diagnostics token; se omite validacion profunda de turneroPilot/publicSync.'
        );
    }
