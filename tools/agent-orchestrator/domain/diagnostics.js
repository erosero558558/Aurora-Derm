...
function buildWarnFirstDiagnostics(input = {}) {
    ...
    if (warnPolicyEnabled(warnPolicyMap, 'retired_executor_active')) {
        const retiredExecutors = Array.isArray(policy?.agents?.retired_executors)
        ...
        const activeRetiredTasks = (Array.isArray(board?.tasks) ? board.tasks : [])
    ...
    const publicSyncJob = Array.isArray(jobsSnapshot)
        ? jobsSnapshot.find((job) => String(job?.key || '') === 'public_main_sync')
        : null;
        ...
        if (!publicSyncJob || publicSyncJob.configured === false) {
            diagnostics.push(
                makeDiagnostic({
                    ...
                    source,
                    message: 'public_main_sync no esta configurado o no pudo verificarse',
                })
            );
        }
    }
    ...
    if (
        ...
        publicSyncJob.verified !== false &&
        (!publicSyncJob.healthy || String(publicSyncJob.state || '') === 'failed')
    ) {
        diagnostics.push(
            makeDiagnostic({
