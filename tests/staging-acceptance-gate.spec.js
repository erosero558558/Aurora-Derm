...
test.describe('Staging acceptance gate V2', () => {
    test('home ES and EN keep the clean shell, locale switch, and critical hooks', async ({ page }) => {
        const cases = [
        ...
        for (const item of cases) {
            ...
            await expect(page.locator('.public-nav__lang')).toHaveAttribute('href', item.switchHref);
            await waitForBookingHooks(page);
            await expect(page.locator('#chatbotWidget')).toHaveCount(1);
        }
    });

    test('language switch preserves equivalent service route', async ({ page }) => {
        await gotoPublicRoute(page, '/es/servicios/acne-rosacea/');
        const switcher = page.locator('.public-nav__lang').first();
        await expect(switcher).toHaveAttribute('href', '/en/services/acne-rosacea/');
        await Promise.all([
    ...
    test('service detail keeps booking preselection through the bridge', async ({ page }) => {
        await gotoPublicRoute(page, '/es/servicios/botox/');
        await waitForBookingHooks(page, 'rejuvenecimiento');
    });

    test('telemedicine booking bridge reaches the booking section', async ({ page }) => {
    ...
    test('mobile keeps key public routes without horizontal overflow', async ({ page }) => {
        await page.setViewportSize({ width: 390, height: 844 });
        const routes = ['/es/', '/en/', '/es/telemedicina/', '/en/telemedicine/'];
