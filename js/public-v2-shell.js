(function () {
    ...
    function bootHeroStage() {
        var roots = Array.from(document.querySelectorAll('[data-stage-carousel]'));
        roots.forEach(function (root) {
            if (!root || root.dataset.stageReady === 'true') return;
            root.dataset.stageReady = 'true';

            var slides = Array.from(root.querySelectorAll('[data-stage-slide]'));
            var triggers = Array.from(root.querySelectorAll('[data-stage-trigger]'));
    ...
    function bootFamilyTabs() {
        ...
        document.querySelectorAll('[data-family-tabs]').forEach(function (nav) {
            ...
            var link = nav.querySelector('[data-family-tab="' + category + '"]');
            if (!link) return;
    ...
    function bootServiceGrid() {
        ...
        document.querySelectorAll('[data-services-grid]').forEach(function (root) {
            if (!root || root.dataset.familyReady === 'true') return;
            root.dataset.familyReady = 'true';
            var section = root.querySelector('[data-family-section="' + category + '"]');
            if (!section) return;
    ...
    function bootCookieBanner() {
        ...
        if (!banner || banner.dataset.publicV2CookieReady === 'true') return;
        banner.dataset.publicV2CookieReady = 'true';
        ...
        function ensureDefaultConsent() {
            if (window.__publicV2ConsentDefaultReady === true) return;
            window.__publicV2ConsentDefaultReady = true;
        ...
        function loadGa4() {
            ...
            if (
                !document.querySelector(
                    'script[data-public-v2-ga4="' + measurementId + '"]'
                )
            ) {
                ...
                script.src =
                    ...
                    measurementId;
                script.dataset.publicV2Ga4 = measurementId;
    ...
    function bootLegacyRuntimeBridge() {
        ...
        if (root.dataset.publicV2RuntimeReady === 'true') return;
        root.dataset.publicV2RuntimeReady = 'true';

        var legacyStylesHref = '/styles.css?v=public-v2-bridge';
        var bootstrapHref = '/js/bootstrap-inline-engine.js?v=public-v2-bridge';
        var runtimeHref = '/script.js?v=public-v2-bridge';
        var booted = false;

        function ensureStyles() {
            if (document.querySelector('link[data-public-v2-legacy="true"]')) return;
            var link = document.createElement('link');
            link.rel = 'stylesheet';
            link.href = legacyStylesHref;
            link.dataset.publicV2Legacy = 'true';
        ...
        function applyServiceHint() {
            ...
            var url = new URL(window.location.href);
            var hint = url.searchParams.get('service') || booking.dataset.serviceHint || '';
            if (!hint) return;

            var match = Array.from(select.options || []).some(function (option) {
        ...
        function appendScript(src, type) {
            if (document.querySelector('script[data-public-v2-src="' + src + '"]')) return;
            var script = document.createElement('script');
            script.src = src;
            script.dataset.publicV2Src = src;
            if (type === 'module') {
                script.type = 'module';
            } else {
                script.defer = true;
        ...
        function boot() {
            if (booted) return;
            booted = true;
            root.dataset.publicV2RuntimeBooted = 'true';
            ensureStyles();
            applyServiceHint();
    ...
    function bootChatFallback() {
        ...
        if (!widget || !container || widget.dataset.publicV2ChatFallbackReady === 'true') {
            return;
        }

        widget.dataset.publicV2ChatFallbackReady = 'true';
        ...
        widget.addEventListener('click', function (event) {
            ...
            var toggle = target.closest('.chatbot-toggle, [data-action="toggle-chatbot"]');
