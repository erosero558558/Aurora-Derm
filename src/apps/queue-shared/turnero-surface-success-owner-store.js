...
const STORAGE_KEY = 'turneroSurfaceSuccessOwnerStoreV1';
const STORAGE_SCHEMA = 'turnero-surface-success-owner-store/v1';
...
function normalizeStatus(value) {
    ...
    if (
        ['active', 'paused', 'inactive', 'standby', 'watch', 'blocked'].includes(
            normalized
        )
    ) {
        return normalized;
...
function normalizeEntry(entry = {}, fallbackScope = 'global') {
    ...
    return {
        id:
            toString(source.id) ||
            `success-owner-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
        ...
        role: toString(source.role, 'success'),
...
export function createTurneroSurfaceSuccessOwnerStore(
    scope = 'regional',
    clinicProfile = null
) {
