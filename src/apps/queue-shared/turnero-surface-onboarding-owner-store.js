...
const STORAGE_KEY = 'turneroSurfaceOnboardingOwnerStoreV1';
const STORAGE_SCHEMA = 'turnero-surface-onboarding-owner-store/v1';
...
function normalizeStatus(value) {
    ...
    if (
        ['active', 'watch', 'blocked', 'standby', 'inactive'].includes(
            normalized
        )
    ) {
        return normalized;
...
function normalizeEntry(entry = {}, fallbackScope = 'global') {
    ...
    return {
        id:
            toString(source.id) ||
            `onboarding-owner-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
...
export function createTurneroSurfaceOnboardingOwnerStore(
    scope = 'global',
    clinicProfile = null
) {
    const normalizedScope = toString(scope, 'global') || 'global';
