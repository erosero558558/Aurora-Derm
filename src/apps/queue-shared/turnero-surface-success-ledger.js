...
const STORAGE_KEY = 'turneroSurfaceSuccessLedgerV1';
const STORAGE_SCHEMA = 'turnero-surface-success-ledger/v1';
...
function normalizeKind(value) {
    const normalized = toString(value, 'followup-note')
        .toLowerCase()
        .replace(/\s+/g, '-');
    return normalized || 'followup-note';
}

function normalizeSignal(value) {
    const normalized = toString(value, 'value').toLowerCase();
    if (
        ['adoption', 'incident', 'value', 'followup', 'feedback', 'note'].includes(
            normalized
        )
    ) {
        return normalized;
...
function normalizeEntry(entry = {}, fallbackScope = 'global') {
    ...
    return {
        id:
            toString(source.id) ||
            `success-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
        ...
        owner: toString(source.owner, 'success'),
...
export function createTurneroSurfaceSuccessLedger(
    scope = 'regional',
    clinicProfile = null
) {
    ...
    return {
        list({ surfaceKey = '', kind = '', status = '', signal = '' } = {}) {
            ...
            const normalizedSignal = toString(signal).toLowerCase();
            return readScopeEntries(STORAGE_KEY, clinicProfile, normalizedScope)
                .filter((entry) => {
                    ...
                    if (normalizedSignal && entry.signal !== normalizedSignal) {
                        return false;
                    }
                    return true;
                })
