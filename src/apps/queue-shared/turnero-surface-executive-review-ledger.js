...
const STORAGE_KEY = 'turneroSurfaceExecutiveReviewLedgerV1';
const STORAGE_SCHEMA = 'turnero-surface-executive-review-ledger/v1';
...
function normalizeKind(value) {
    const normalized = toString(value, 'review-item')
        .toLowerCase()
        .replace(/[\s_]+/g, '-');

    if (normalized === 'review' || normalized === 'item') {
        return 'review-item';
    }

    return normalized || 'review-item';
}
...
function normalizeEntry(entry = {}, fallbackScope = 'regional') {
    ...
    return {
        ...
        title: toString(source.title || source.label, 'Executive review item'),
...
export function createTurneroSurfaceExecutiveReviewLedger(
    scope = 'regional',
    clinicProfile = null
) {
    const normalizedScope = resolveTurneroSurfaceExecutiveReviewScope(scope);
    ...
    return {
        ...
        add(entry = {}) {
            const nextEntry = normalizeEntry({
                ...
                kind: entry.kind || 'review-item',
                title: entry.title || 'Executive review item',
...
export default createTurneroSurfaceExecutiveReviewLedger;
