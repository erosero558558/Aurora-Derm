...
const STORAGE_KEY = 'turneroSurfacePackageOwnerStoreV1';
const STORAGE_SCHEMA = 'turnero-surface-package-owner-store/v1';
...
function normalizeScope(scope, clinicProfile) {
    return resolveTurneroSurfacePackageScope(scope, clinicProfile);
}
...
function normalizeEntry(entry = {}, fallbackScope = 'regional') {
    ...
    return {
        id:
            toString(source.id) ||
            `package-owner-${Date.now()}-${Math.random()
                .toString(36)
                .slice(2, 8)}`,
        scope,
        ...
        actor,
        owner: actor,
        role: toString(source.role || 'package', 'package'),
...
export function createTurneroSurfacePackageOwnerStore(
    scope = 'regional',
    clinicProfile = null
) {
    const normalizedScope = normalizeScope(scope, clinicProfile);
    ...
    return {
        list({ surfaceKey = '', role = '', status = '' } = {}) {
            const normalizedSurfaceKey = toString(surfaceKey)
                ? normalizeTurneroSurfacePackageSurfaceKey(surfaceKey)
                : '';
