...
const STYLE_ID = 'turneroSurfaceOnboardingBannerInlineStyles';
...
function resolveReadout(input = {}) {
    ...
    const ledger = Array.isArray(input.ledger)
        ? input.ledger
        : Array.isArray(pack.ledger)
          ? pack.ledger
          : [];
    const owners = Array.isArray(input.owners)
        ? input.owners
        : Array.isArray(pack.owners)
          ? pack.owners
          : [];
    ...
    return (
        input.readout ||
        pack.readout ||
        buildTurneroSurfaceOnboardingReadout({
            snapshot,
            gate,
            checklist,
            ledger,
...
export function buildTurneroSurfaceOnboardingBannerHtml(input = {}) {
    const readout = resolveReadout(input);
    if (toString(readout.gateBand, 'blocked') === 'ready') {
        return '';
    }

    const title = toString(input.title, readout.title || 'Surface onboarding');
    const eyebrow = toString(input.eyebrow, 'Surface onboarding');
    ...
    return `
        ...
        >
            <div class="turnero-surface-ops__banner-copy">
                ...
                <p>${escapeHtml(readout.summary || 'Onboarding visible.')}</p>
...
export function mountTurneroSurfaceOnboardingBanner(target, input = {}) {
    ...
    ensureBannerStyles();
    root.innerHTML = buildTurneroSurfaceOnboardingBannerHtml(input);
    return root.querySelector('.turnero-surface-onboarding-banner');
}
