...
export async function logoutSession() {
    ...
    const pinMode =
        ...
        normalizePinMeta(auth?.turneroOperatorAccessMeta) !== null ||
        String(auth?.authMethod || '') === 'operator_pin';
    const preserveOpenClawSurface =
        !pinMode &&
        (isOpenClawOperatorMode(auth) ||
            normalizeMode(auth?.recommendedMode) === 'openclaw_chatgpt' ||
            normalizeMode(auth?.loginSurfaceMode) === 'openclaw_chatgpt' ||
            normalizeTransport(auth?.transport, '') !== '' ||
            normalizeTransport(auth?.openClawSnapshot?.transport, '') !== '');

    if (!pinMode) {
        await logoutOpenClawSession();

        if (
            preserveOpenClawSurface &&
            !isOpenClawOperatorMode(getState().auth)
        ) {
            const nextAuth = getState().auth || {};
            applyOpenClawPayload({
                authenticated: false,
                configured: nextAuth.configured !== false,
                mode: 'openclaw_chatgpt',
                recommendedMode: 'openclaw_chatgpt',
                status:
                    String(nextAuth.status || 'anonymous').trim() ||
                    'anonymous',
                transport: normalizeTransport(
                    nextAuth.transport,
                    normalizeTransport(
                        nextAuth.openClawSnapshot?.transport,
                        'local_helper'
                    )
                ),
                challenge:
                    normalizeChallenge(nextAuth.challenge) ||
                    normalizeChallenge(nextAuth.openClawSnapshot?.challenge),
                redirectUrl: String(
                    nextAuth.redirectUrl ||
                        nextAuth.openClawSnapshot?.redirectUrl ||
                        ''
                ).trim(),
                expiresAt: String(
                    nextAuth.attemptExpiresAt ||
                        nextAuth.openClawSnapshot?.expiresAt ||
                        ''
                ).trim(),
                error: String(
                    nextAuth.lastError ||
                        nextAuth.openClawSnapshot?.lastError ||
                        ''
                ).trim(),
            });
        }
